...
public class DlFragment extends BaseFragment{
    ...
    public void initData(){
        ...
        //添加数据
//        setData(100, 15);
//        ToastUtil.TextToast(SsjcActivity.mlist.size()+"");
        setData();
...
//            LineData data = new LineData(quarterStrs,allLinesList);
    ...
    private void setData() {
        ...
        for (int i = 0; i < SsjcActivity.mlist.get(0).getSData().get(0).getValue().size(); i++) {
           yVals1.add(new Entry(i, SsjcActivity.mlist.get(0).getSData().get(0).getValue().get(i)));
        }

        ArrayList<Entry> yVals2 = new ArrayList<Entry>();

        for (int i = 0; i < SsjcActivity.mlist.get(0).getSData().get(1).getValue().size(); i++) {
            yVals2.add(new Entry(i, SsjcActivity.mlist.get(0).getSData().get(1).getValue().get(i)));
        }

        ArrayList<Entry> yVals3 = new ArrayList<Entry>();

        for (int i = 0; i < SsjcActivity.mlist.get(0).getSData().get(2).getValue().size(); i++) {
            yVals3.add(new Entry(i, SsjcActivity.mlist.get(0).getSData().get(2).getValue().get(i)));
        }
        ...
        if (mChart.getData() != null &&
            ...
            // create a dataset and give it a type
            set1 = new LineDataSet(yVals1, "A相电流(A)");
            set2 = new LineDataSet(yVals2, "B相电流(A)");
            set3 = new LineDataSet(yVals3, "C相电流(A)");
            setLineDataSet(set1,getResources().getColor(R.color.chart_1));
            setLineDataSet(set2,getResources().getColor(R.color.chart_2));
            setLineDataSet(set3,getResources().getColor(R.color.chart_3));
