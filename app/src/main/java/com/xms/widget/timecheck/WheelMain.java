package com.xms.widget.timecheck;
...
public class WheelMain {
    ...
    private boolean hasSelectTime;//是否显示日期
    private int hasSelectMonth = 0;//是否只显示年月
    ...
    public WheelMain(View view, boolean hasSelectTime, int isnotny) {
        super();
        this.view = view;
        this.hasSelectTime = hasSelectTime;
        this.hasSelectMonth = isnotny;
        setView(view);
    }
    ...
    public String getTime(String format) {
        ...
        String year = String.format(format,
                (wv_year.getCurrentItem() + START_YEAR));
        String month = String.format(format, ((wv_month.getCurrentItem() + 1)));
        String day = String.format(format, (wv_day.getCurrentItem() + 1));
        if (hasSelectMonth!=0){
            switch (hasSelectMonth){
                case 1:
                    sb.append(year);
            ...
            if (!hasSelectTime)
                ...
                String hours = String.format(format, wv_hours.getCurrentItem());
                String mins = String.format(format, wv_mins.getCurrentItem());
                sb.append(year).append("-").append(month).append("-").append(day)
                        .append(" ").append(hours).append(":").append(mins);
            }
        }
