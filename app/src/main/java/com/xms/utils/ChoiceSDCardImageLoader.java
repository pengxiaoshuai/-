 ...
 * @date 2016年1月26日
 */
public class ChoiceSDCardImageLoader {
	// 缓存
	...
	public ChoiceSDCardImageLoader(int screenW, int screenH) {
	...
	public void loadImage(int smallRate, final String filePath,
		...
		Bitmap bmp = null;
		if (filePath != null) {
			if (!filePath.equals("tianjia")) {
				bmp = loadDrawable(smallRate, filePath, new ImageCallback() {
					...
					public void imageLoaded(Bitmap bmp) {
						if (imageView.getTag().equals(filePath)) {
							if (bmp != null) {
								...
								imageView
										.setImageResource(R.mipmap.choice_empty_photo);
							}
						}
					}
				});
				if (bmp != null) {
					...
					imageView.setImageResource(R.mipmap.choice_empty_photo);
				}
			} else {
